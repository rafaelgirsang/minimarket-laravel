...
@section('content')
                    ...
                    <div class="menu menu-sub menu-sub-dropdown w-250px w-md-300px" data-kt-menu="true"
                        ...
                        <div class="px-7 py-5">
                                    ...
                                    <div class="form-floating">
                                        <select class="form-select" id="floatingSelect"
                                            ...
                                            <option {{ crypt_open(request('searchBy')) == '' ? 'Selected' : '' }}
                                                value="">Pilih</option>
                                            <option {{ crypt_open(request('searchBy')) == 'akun' ? 'Selected' : '' }}
                                                value="{{ crypt_make('akun') }}">Akun</option>
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="{{ url('jurnal/akun') }}" type="reset"
    ...
    <div class="modal fade" tabindex="-1" id="create">
        <form action="{{ url('jurnal/storeAkun') }}" method="POST">
            @csrf
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <label for="basic-url" class="form-label">Saldo Normal</label>
                                <div class="input-group input-group-sm  mb-5">
                                    <select class="form-select" id="floatingSelect"
                                        aria-label="Floating label select example" style="font-size:1rem"
                                        name="saldo_normal" required>
    ...
    @foreach ($data as $row)
        <div class="modal fade" tabindex="-1" id="update{{ $row->id }}">
            <form action="{{ url('jurnal/updateAkun/' . $row->id) }}" method="post">
                @csrf
                @method('PUT')
                                        ...
                                        <select class="form-select" id="floatingSelect"
                                            ...
                                            @foreach ($kategori as $rowKategori)
                                                <option value="{{ $rowKategori->id }}"
                                                    {{ $rowKategori->id == $row->kategori_id ? 'Selected' : '' }}>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group input-group-sm  mb-5">
                                        <select class="form-select" id="floatingSelect"
                                            ...
                                            <option value="">Pilih</option>
                                            <option value="DEBIT" {{ $row->saldo_normal == 'DEBIT' ? 'Selected' : '' }}>
                                                DEBIT
                                            </option>
                                            <option value="KREDIT" {{ $row->saldo_normal == 'KREDIT' ? 'Selected' : '' }}>
                                                KREDIT
                                            </option>
